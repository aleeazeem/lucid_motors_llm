...
class WebCrawler:
    ...
    def __init__(self, configs: Dict[str, Any]):
        self.start_url = configs['configs']['base_url'] + configs['configs']['paths']
        self.max_depth = configs['crawler']['max_depth']
        self.max_workers = configs['crawler']['max_workers']
        ...
        self.domain_name = (configs['configs']['name'].replace(' ', '_')).lower() # e.g., "wells_fargo"
        output_base_dir = configs['configs']['output_path'] # e.g., "output"
        self.output_dir = os.path.join(output_base_dir, self.domain_name) # e.g., "output/wells_fargo"
        self.pdf_output_dir = os.path.join(self.output_dir, 'pdfs') # e.g., "output/wells_fargo/pdfs"
        ...
        log.info(f"PDF directory: {self.pdf_output_dir}")
    ...
    def crawl_page(self, url, depth, parent_url):
        ...
        try:
            ...
            if depth < self.max_depth:
                ...
                log.info(f"  Found {len(links)} new links at depth {depth}")
    ...
    def crawl(self):
        ...
        for depth in range(self.max_depth + 1):
            ...
            log.log_step(f"Processing depth {depth} - {len(current_level)} URLs")
            ...
            log.info(f"Completed depth {depth}. Found {len(next_level)} URLs for next level.")
    ...
    def save_results(self, json_filename=None, txt_filename=None):
        ...
        if txt_filename is None:
            domain_name = self.domain.replace('.', '_')
            txt_filename = f"{domain_name}.txt"
        
        # Create full paths using self.output_dir instead of self.domain_name
...
if __name__ == "__main__":
    ...
    log.log_pipeline("Starting Web Crawling Process")
    config_loader = ConfigLoader(config_path="configs/data_processing.yaml")
    crawler = WebCrawler(config_loader.configs)
    log.log_messages([f"Start URL: {crawler.start_url}",
                      f"Max Depth: {crawler.max_depth}",
                     f"Max Workers: {crawler.max_workers}",
                     f"Output Directory: {crawler.output_dir}",
                     f"PDF Directory: {crawler.pdf_output_dir}"])
    ...
    log.log_pipeline("Web Crawling Process Completed")